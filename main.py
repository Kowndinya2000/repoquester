        ...
        for repos in os.listdir():
            if(repos == repoName):
                ...
                if(num_commits > 1):
                    ...
                    end1 = datetime(int(today[0]),int(today[1]),int(today[2]))
                    #state_days = -1
                    # for d in arrow.Arrow.range('day', end1, end):
                    #     state_days += 1
                    # if(state_days > -1):
                    #     state = 'active'
...
def dependency():
    ...
    try:
        ...
        for repos in os.listdir():
            if(repos == repoName):
                ...
                for filename in AbsFiles:
                    ...
                    fp = open(filename,'r')
                    for x in fp:
                        if("0." in x or "1." in x or "2." in x or "3." in x or "4." in x or "5." in x or "6." in x or "7." in x or
                            "8." in x or "9." in x):
        ...
        if(totalLines > 1):
            ...
            os.chdir(root_directory)
            val = (str(dependency_ratio_total),repo_id)
            QUERY = '''UPDATE `repoquester_results` SET `dependency`=? WHERE `repo_id`=?'''
            try:
                conn = sqlite3.connect('repo_quester.db',timeout=180)
                conn.execute(QUERY,val)
                conn.commit()
                conn.close()
                print('DEPENDENCY: ',dependency_ratio_total)
            except Exception as ex:
                print(ex)
                conn = sqlite3.connect('repo_quester.db',timeout=180)
                conn.execute(QUERY,val)
                conn.commit()
                conn.close()
                print('DEPENDENCY: ',dependency_ratio_total,' trying again.. ok')
    except Exception as ex:
        print(ex)
def prs():
    os.chdir(root_directory)
    ...
    try:
        ...
        for token in git_tokens:
            if(flag1 == True):
                ...
                try:
                    stargazers_count = requests.get(URL).json()['stargazers_count']
                    flag1 = True
                    break
                except:
                    continue
        if(flag1 == False):
            try:
                stargazers_count = requests.get(URL).json()['stargazers_count']
                print('without token - stars - fetch ok')
            except Exception as ex:
                print(ex)
        if(stargazers_count > 0):
            os.chdir(root_directory)
            val = (str(stargazers_count),repo_id)
            QUERY = '''UPDATE `repoquester_results` SET `stars`=? WHERE `repo_id`=?'''
            try:
                os.chdir(root_directory)
                conn = sqlite3.connect('repo_quester.db',timeout=180)
                conn.execute(QUERY,val)
                ...
                conn.commit()
                conn.close()
                print("Stars: ",stargazers_count,' trying again.. ok')
    except Exception as ex:
        print(ex)
...
def main():
    ...
    processes.append(p)
    p.start()
    p = multiprocessing.Process(target=dependency,args=())
    processes.append(p)
    p.start()
    ...
    processes.append(p)
    p.start()
    p = multiprocessing.Process(target=stargazers,args=())
    processes.append(p)
    p.start()
